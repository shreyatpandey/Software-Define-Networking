		...
		if(FD_ISSET(tun_fd,&fs))
		{
			...
			{	
				...
				if(icmp_pac->type== ICMP_ECHO)
				{
					...
					/* Update Octane Message */
					if(stage>3)
					{
						ip_udptu->protocol = 253;
						uint8_t octanepacket[40] = {0};
						octanemessage->serializeoctanemessage(buffertunnel,(char *)octanepacket);
						//ip_udptu->protocol = 253;
						int send_to_secondary_octane =  sendto(socket_primary_udp,(char *)octanepacket,sizeof(octanepacket),0,(struct sockaddr*)&primaryaddr,length_udp_socket);
						cancellationpoint(send_to_secondary_octane,"Error in sending octane message from primary udp to secondary udp ");
